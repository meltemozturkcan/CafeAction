...
namespace SignalRWebUI.Controllers
{
    public class CouponCodeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responMessage = await client.GetAsync("https://localhost:7276/api/CouponCode");
        ...
        public async Task<IActionResult> CreateCode(CreateCoponCodeDto  createCoponCodeDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7276/api/CouponCode", strContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7276/api/CouponCode/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7276/api/CouponCode/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateAbout(UpdateCoponCodeDto  updateCoponCodeDto)
        {
            ...
            StringContent strContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7276/api/CouponCode", strContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
