...
namespace SignalRWebUI.Controllers
{
	public class NotificationsController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			...
			var responMessage = await client.GetAsync("https://localhost:7276/api/Notifications");
		...
		public async Task<IActionResult> CreateNotifications(CreateNotificationDto createNotificationDto)
		{
			...
			var responseMessage = await client.PostAsync("https://localhost:7276/api/Notifications", strContent);
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync($"https://localhost:7276/api/Notifications/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync($"https://localhost:7276/api/Notifications/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
		...
		public async Task<IActionResult> UpdateNotifications(UpdateNotificationDto  updateNotificationDto)
		{
			...
			StringContent strContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
			var responseMessage = await client.PutAsync("https://localhost:7276/api/Notifications", strContent);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.GetAsync($"https://localhost:7276/api/Notifications/ChangeNotificationStatusTrue/{id}");
            return RedirectToAction("Index");
        }
        public async Task<IActionResult> UpdateNotificationToFalse(int id)
        {
            var client = _httpClientFactory.CreateClient();
            await client.GetAsync($"https://localhost:7276/api/Notifications/ChangeNotificationStatusFalse/{id}");
            return RedirectToAction("Index");
        }
