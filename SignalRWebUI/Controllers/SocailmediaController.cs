...
namespace SignalRWebUI.Controllers
{
    public class SocailmediaController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responMessage = await client.GetAsync("https://localhost:7276/api/SocialMedia");
        ...
        public async Task<IActionResult> CreateSocailMedia(CreateSocialMediaDto createSocialMediaDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7276/api/SocialMedia", strContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7276/api/SocialMedia/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7276/api/SocialMedia/{id}");
        ...
        public async Task<IActionResult> UpdateSocailMedia(UpdateSocialMediaDto updateSocialMediaDto)
        {
            ...
            StringContent strContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7276/api/SocialMedia", strContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
